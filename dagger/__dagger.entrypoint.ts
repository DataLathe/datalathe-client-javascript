...
import { DatalatheClientJs } from "./src/index"
...
function rebuildDatalatheClientJs(state: any): DatalatheClientJs {
  const __obj = Object.assign(Object.create(DatalatheClientJs.prototype), state ?? {})
  if (state) {
...
async function serializeDatalatheClientJs(__obj: DatalatheClientJs): Promise<any> {
...
async function register(): Promise<string> {
  let mod = dag.module_()
  let obj_DatalatheClientJs = dag.typeDef().withObject("DatalatheClientJs", { sourceMap: dag.sourceMap("src/index.ts", 6, 14) })
  obj_DatalatheClientJs = obj_DatalatheClientJs.withFunction(dag.function_("integration", dag.typeDef().withKind(TypeDefKind.VoidKind).withOptional(true)).withDescription("Workspace check: the full chip-lifecycle integration suite against an\nephemeral engine + chip-manager built from the pinned datalathe-backend.").withSourceMap(dag.sourceMap("src/index.ts", 19, 9)).withCheck())
  obj_DatalatheClientJs = obj_DatalatheClientJs.withConstructor(dag.function_("", obj_DatalatheClientJs).withArg("ws", dag.typeDef().withObject("Workspace").withOptional(true), { sourceMap: dag.sourceMap("src/index.ts", 9, 15) }))
  mod = mod.withObject(obj_DatalatheClientJs)
  return await mod.id()
}
...
async function invoke(
  ...
  switch (parentName) {
    case "DatalatheClientJs": {
      switch (fnName) {

        case "": {
          const __arg_ws = args["ws"] === undefined || args["ws"] === null ? args["ws"] : __loadCoreObject(args["ws"], "Workspace")
          const __result = await new DatalatheClientJs(__arg_ws) as unknown as DatalatheClientJs
          return await serializeDatalatheClientJs(__result)
        }

        case "integration": {
          const __parent = rebuildDatalatheClientJs(parentJson)
          const __result = await __parent.integration()
          return __result
        }
        default:
          throw new Error(`unknown function ${fnName} on DatalatheClientJs`)
      }
    }
    default:
